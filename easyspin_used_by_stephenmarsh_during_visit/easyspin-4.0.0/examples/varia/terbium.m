...
ThSys = struct('S',7/2,'g',2.0136,...
         'B40',b,'B44',[5*b 0],'B60',c,'B64',[-21*c 0])

% Computer the energy level diagram using EasySpin's level() function
N = 200;
maxB = 2500;
extB = linspace(0,maxB,N); % mT
E = levels(ThSys,0,0,extB)/1e3;

% Plot the energy level diagram
h = plot(extB,E,'k'); set(h,'LineWidth',2);
xlabel('magnetic field [mT]'); ylabel('energy [GHz]');

Params.Range = [0 maxB];
Params.Orientations = [0;0]; % orientation
