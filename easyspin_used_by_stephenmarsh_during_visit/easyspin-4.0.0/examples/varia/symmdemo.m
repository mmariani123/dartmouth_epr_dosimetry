...
% If A is tilted along the first Euler angle
% (i.e. rotated around the z axis) it remains axial.

Sys.Apa = [56 0 0]*pi/180;
symm(Sys)
...
Sys.Apa = [12 0 0];
fprintf('First Euler angle non-zero: %s\n',symm(Sys));

Sys.Apa = [0 -61 0];
fprintf('Second Euler angle non-zero: %s\n',symm(Sys));

Sys.Apa = [14 73 0];
fprintf('First two Euler angle non-zero: %s\n',symm(Sys));
